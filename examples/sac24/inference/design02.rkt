...
(require lightstep/base lightstep/syntax lightstep/inference
         (only-in "design01.rkt" LAM [subst LAM:subst] -->LAM-rule))
(provide PCF₀ subst -->PCF₀-rules)
...
(define-inference (-->PCF₀-rules) #:super [(-->LAM-rule)]
  [--------------------------- "add"
   `((+ ,N₁ ,N₂) → ,(+ N₁ N₂))      ]
...
(module+ test
  ...
  (check-equal? (-->PCF₀ '(if (<= 10 8) 3 2)) (set 3)) ;; これもまだだが
                                                       ;; まちがって遷移してる
  #;
...
(define-inference (-->PCF₁-rule)
  #:do [(define →PCF₀ (reducer-of (-->PCF₀-rules)))]
